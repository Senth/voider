...
/**
 * /** Renders points, lines, rectangles, filled rectangles and boxes. </p> This class works with
 * OpenGL ES 1.x and 2.0. In its base configuration a 2D orthographic projection with the origin in
 * the lower left corner is used. Units are given in screen pixels. </p> To change the projection
 * properties use the {@link #setProjectionMatrix(Matrix4)} method. Usually the {@link
 * Camera#combined} matrix is set via this method. If the screen orientation or resolution changes,
 * the projection matrix might have to be adapted as well. </p> Shapes are rendered in batches to
 * increase performance. The standard use-pattern looks as follows:
 * <p>
 * <pre>
 * {@code
 * camera.update();
 ...
 * </pre>
 * <p>
 * The class has a second matrix called the transformation matrix which is used to rotate, scale and
 * translate shapes in a more flexible manner. This mechanism works much like matrix operations in
 * OpenGL ES 1.x. The following example shows how to rotate a rectangle around its center using the
 ...
 * then translate in the above example). The projection and transformation matrices are a state of
 * the ShapeRenderer, just like the color and will be applied to all shapes until they are changed.
 * @author mzechner, stbachmann A more advanced shape renderer which allows custom shaders and lines
 *         with multicolors.
 */
...
/**
 * Translation coordinates, uses this so we don't have to flush every time we translate :)
 */
...
/**
 * Easier method for drawing a triangle if one has the vertices in Vector2 format.
 * @param triangle an array with at least 3 vertices, will only draw the three first vertices.
 */
...
/**
 * Adds these coordinate to the translate vector. This doesn't affect the transformation matrix.
 * @param x
 * @param y
 * @param z
 */
...
/**
 * Adds this vector to the translate vector. This doesn't affect the transformation matrix.
 * @param vector
 */
 ...
 * @param unused only used to make the method different than {@link #rect(float, float, float,
 * float)}
 */
...
/**
 * Draws a rectangle in the x/y plane. The x and y coordinate specify the bottom left corner of the
 * rectangle. The {@link ShapeType} passed to begin has to be {@link ShapeType#Filled} or {@link
 * ShapeType#Line}.
 * @param x
 * @param y
 * @param width
...
/**
 * Draws a 3D-box. The x, y and z coordinate specify the bottom left front corner of the rectangle.
 * The {@link ShapeType} passed to begin has to be {@link ShapeType#Line}.
 * @param x
 * @param y
 * @param z
...
/**
 * Calls {@link #circle(float, float, float, int)} by estimating the number of segments needed for a
 * smooth circle.
 * @param x
 * @param y
 * @param radius
...
/**
 * Calls {@link #cone(float, float, float, float, float, int)} by estimating the number of segments
 * needed for a smooth circular base.
 * @param x
 * @param y
 * @param z
 * @param radius
...
/**
 * Pushes the shape type, i.e. will end current rendering type and start to draw using this one.
 * {@link #pop()} will return to the previous rendering type
 ...
 * if {@link ShapeType#Point} is specified, only call #point(). In case OpenGL ES 1.x is used, the
 * projection and modelview matrix will be modified.
 */
private void begin() {
	if (mCurrentType.isEmpty()) {
...
/**
 * Pops the current rendering type and start to draw the previous one. If no previous rendering type
 * exist.
 */
public void pop() {
	if (mCurrentType.isEmpty()) {
