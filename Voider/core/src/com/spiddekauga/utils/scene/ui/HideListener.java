...
/**
 * Hides specified GUI elements when either one or several buttons is enabled/disabled If several
 * buttons have been added, if any of them are checked (if showWhenChecked is true, otherwise if one
 * of them isn't checked) the toggle actors will be shown
 */
...
/**
 * Creates a hide listener, listens to the specified button if it's enabled or not. Calls onHide()
 * after the actors get hidden, and onShow() after they are shown.
 * @param showWhenChecked set to true to show the actors when this is checked. Set to false to show
 * the actors when the button is unchecked.
...
/**
 * Creates a hide listener. Calls onHide() after the actors get hidden, and onShow() after they are
 * shown.
