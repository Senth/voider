...
/**
 * Executes the specified action. Only pushes this command onto the undo stack if it was successful.
 * Can set the commandn as chained. Chained commands will also execute the previous command on an
 * undo(), and if there exist a chained command directly after a redo() that will also be redone
 * until the next redo isn't a chained command. If this command is an instance of #ICommandStackable
 * and is the same type of command as the last executed command it will combine these two.
...
/**
 * Executes the specified action. Only pushes this command onto the undo stack if it was
 * successful.
 * @param command the command to execute
 * @return true if the command was executed successfully
...
/**
 * Push a delimiter onto the execute (undo) stack
 * @param name the name of the delimiter. Useful when undoing to a specific delimiter using
 */
...
/**
 * Undo all commands to and including the specified delimiter. This method adds all undone commands
 * to the redo stack.
...
/**
 * Undoes the last executed command. The commands are on a stack so this can be called multiple
 * times in a row. This adds the command to the redo stack
...
/**
 * Redoes the last undone command. The commands are on a stack so this can be called multiple times
 * in a row.
 */
public void redo() {
	boolean chained = true;
...
/**
 * @return last executed command. Not undo commands. E.g. if C1 is executed, then C2, then undo,
 * this will return C1.
 */
