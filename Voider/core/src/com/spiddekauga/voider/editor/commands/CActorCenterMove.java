...
/**
 * Moves the center offset to the specified position. This command also takes an optional actor
 * which will be moved in the opposite direction; this will cause it to look as it wasn't moved at
 * all.
 */
...
/**
 * Creates a command that will move the center of the actor definition. It will also update the
 * actors position. Note that the actors position must be its position where it was at oldCenter.
 * @param actorDef the actor definition which center to move
 * @param newCenter the new center position
 * @param oldCenter old center position, used for restoring using undo
 * @param actorEditor the actor editor to send a onActorChange() event to
...
public boolean execute() {
	mActorDef.getShape().setCenterOffset(mCenterNew);
...
@Override
public boolean undo() {
	mActorDef.getShape().setCenterOffset(mCenterOld);
