...
/**
 * Runs the provided SQL and returns the same {@link DatabaseCursor} that was passed to this method.
 * Use this method when you want to avoid reallocation of {@link DatabaseCursor} object. Note that
 * you shall only pass the {@link DatabaseCursor} object that was previously returned by a rawQuery
 * method. Creating your own {@link DatabaseCursor} and then passing it as an object will not work.
...
/**
 * Set if this instance should queue failed SQL statements when calling {@link #execSQL(String)}.
 ...
 * If {@link #setQueueFailedSqlStatements(boolean)} has been called with true and if the database
 * isn't yet connect the SQL statement will be pushed onto a queue; when this method is called again
 * it will retry to execute the queue messages before the new SQL statement.
