 ...
 * @return indices of the corners to remove to fix the area problem, empty if no solution was found
 */
 ...
 * @return area and indices of the fixed polygon. Indices will be -1 if this method failed to find
 * any solutions
 */
...
/**
 * Calculates the triangle area. Uses this algorithm: \code (a) (b) (c) | Ax(By - Cy) + Bx(Cy - Ay)
 * + Cx(Ay - By) | area = | ——————————————————————————————————————— | | 2 | \endcode
...
/**
 * Validates if the triangle is OK, else it tries to fix it. If it can't fix the triangle an
 * exception is thrown.
...
/**
 * Checks and returns the first occurrence of vertices that are too close. Run this multiple times
 * to check for all vertices
 * @param vertices the vertices to check
 * @return two vertices that are too close to each other, null if all vertices are at OK distance
 * from each other
 */
...
/**
 * Calculates the area of a polygon but skips all between low and high indices (not including)
 * @param vertices all vertices of the polygon
 ...
 * @return velocity of the object needed to intercept the object. Returns Vector2(NaN,NaN) if the
 * object cannot intercept the target (because of speed). Be sure to free the returning variable
 * using Pools.vector2.free(velocity);
 */
 ...
 * @return list with all the vertices for the line. These have been created with Vector2Pool, make
 * sure to free them afterwards :)
 */
...
/**
 * Calculates the point of intersection between two lines. Note that the lines are treated as
 * infinite, so the lines don't have to intersect.
 ...
 * @see #linesIntersect(Vector2, Vector2, Vector2, Vector2) if the point is actually inside the
 * lines
 ...
 * @author andreasdr http://stackoverflow.com/questions/2049582/how-to-determine-a-point
 * -in-a-triangle
 */
 ...
 * @param point the point to test whether it inside the triangle or not
 * @param triangle three vertices that makes triangle
 * @param triangleStartIndex starting index of the triangle this can be used when the triangle
 * already is within another larger list.
 * @return true if the point is inside the triangle
 * @author andreasdr http://stackoverflow.com/questions/2049582/how-to-determine-a-point
 * -in-a-triangle
 */
...
/**
 * Calculates the direction between two points. Reuses an old Vector2 instead of creating a new
 * one.
 * @param fromPoint from this point
 * @param toPoint to this point
 ...
 * @return vector with all border vertices, null if corner size and border corner size isn't the
 * same
 */
...
/**
 * Rotates all vertices after temporarily moving it to another place (usually the center)
 * @param vertices the vertices to rotate
 * @param degrees how many degrees to rotate the vertices
 * @param containsReferences set to true if the vertices contains references thus we don't have to
 * recalculate all vertices
 * @param offset move the vertices temporarily here before rotating (this offset is never changed)
 */
 ...
 * @param vertices the vertices to check
 * @return vertex farthest away from the specified point, if vertices are empty a null vertex will
 * be returned.
 */
...
/**
 * Tests whether the polygon shape with intersections only contains simple intersections, i.e.
 * intersections that won't make a hole inside the polygon.
 ...
 * itself. Will only work for simple intersections, i.e. intersections that won't make a hole inside
 * the polygon. To try if it's valid use
 * @param vertices all the corner vertices of the polygon, this array will be modified if an
 * intersection exists {@link #arePolygonIntersectionsSimple(List, List)}
 * @param testLoop this will include a test from end to begin vertices. If the polygon isn't
 * complete you might not want to test this.
 * @return newly created vertices, these have been created from Pools.vector2 be sure to free them
 * after their use. Same goes for the ArrayList. Null if no vertices were created.
 */
 ...
 * @param testToIndex only tests to the specified index (lines after and including this is not
 * tested)
 * @return index of the other line that intersects with lineIndex, -1 if no intersection exists.
 */
...
/**
 * Checks if two lines intersects, but skips the vertex points of the lines (i.e. if a point is the
 * same, it simply skips the calculation. Note that the lines might still intersect, e.g. LINE1:
 * (0,0) - (5,0), LINE2: (0,0) - (2,0).
...
/**
 * Checks if two lines intersects. They only intersect if the intersection point is within the
 * lines' range (i.e. these are not infinite lines)
 * @param line1a first vertex point for line 1
 * @param line1b second vertex point for line 1
 * @param line2a first vertex point for line 2
 * @param line2b second vertex point for line 2
 ...
 * @author CommanderKeith on http://Java-Gaming.org
 * @see #linesIntersectNoCorners(Vector2, Vector2, Vector2, Vector2)
 * @see #getLineLineIntersection(Vector2, Vector2, Vector2, Vector2) to get the point of
 * intersection
 */
...
/**
 * Splits a polygon with intersections into smaller polygons. This only works if the polygon
 * contains simple intersections, i.e. test it with
 ...
 * @return list of polygons that were split from the original polygon. Don't forget to free the
 * array lists!
 */
 ...
 * @author Grumdrig http://stackoverflow.com/167531/grumdrig Question: http://stackoverflow
