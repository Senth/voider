...
/*******************************************************************************
 * Copyright 2011 See AUTHORS file. Licensed under the Apache License, Version 2.0 (the "License");
...
/**
 * A simple implementation of the ear cutting algorithm to triangulate simple polygons
 * without holes. For more information:
 * <ul>
 * <li><a
 * href="http://cgm.cs.mcgill.ca/~godfried/teaching/cg-projects/97/Ian/algorithm2.html"
 * >http://cgm.cs.mcgill.ca/~godfried/teaching/cg-projects/97/Ian/algorithm2.html</a></li>
 * <li><a
 * href="http://www.geometrictools.com/Documentation/TriangulationByEarClipping.pdf"
 * >http://www.geometrictools.com/Documentation/TriangulationByEarClipping.pdf</a></li>
 * </ul>
...
/**
 * Triangulates the given (concave) polygon to a list of triangles. The resulting
 * triangles have clockwise order.
 * @param polygon the polygon
 * @return the triangles.
 * @throws PolygonComplexException if the polygon is complex, i.e. can't triangulate
 *         the polygon
 */
public ArrayList<Vector2> computeTriangles(final List<Vector2> polygon) {
	final ArrayList<Vector2> triangles = new ArrayList<>();
	final ArrayList<Vector2> vertices = new ArrayList<>();
	...
	while (vertices.size() > 3) {
		...
		// ear change! --> Improve
		final int vertexTypes[] = classifyVertices(vertices);
		...
		/*
		 * ESpitz: If there are only three verts left to test, or there were only three
		 * verts to begin with, we have the final triangle.
		 */
	if (vertices.size() == 3) {
 ...
 *         {@link EarClippingTriangulator#CONCAVE} or
...
private int[] classifyVertices(final ArrayList<Vector2> pVertices) {
	...
	for (int index = 0; index < vertexCount; index++) {
		...
		if (EarClippingTriangulator.isTriangleConvex(previousVertex.x, previousVertex.y, currentVertex.x, currentVertex.y, nextVertex.x,
				nextVertex.y)) {
...
private boolean isEarTip(final ArrayList<Vector2> pVertices, final int pEarTipIndex, final int[] pVertexTypes) {
	if (this.concaveVertexCount != 0) {
		...
		if (EarClippingTriangulator.isAnyVertexInTriangle(pVertices, pVertexTypes, previousVertex.x, previousVertex.y, currentVertex.x,
...
/**
 * @return <code>true</code> when the Triangles contains one or more vertices,
 *         <code>false</code> otherwise.
 */
private static boolean isAnyVertexInTriangle(final ArrayList<Vector2> pVertices, final int[] pVertexTypes, final float pX1, final float pY1,
...
private static int computeSpannedAreaSign(final float pX1, final float pY1, final float pX2, final float pY2, final float pX3, final float pY3) {
		/*
		 * Espitz: using doubles corrects for very rare cases where we run into floating
		 * point imprecision in the area test, causing the method to return a 0 when it
		 * should have returned -1 or 1.
		 */
	double area = 0;
