...
/**
 * Base speed of the level, the actual level speed may vary as it can be changed by triggers
 */
@Tag(83)
private float mSpeed = ConfigIni.getInstance().editor.level.getSpeedDefault();
...
/**
 * Calculates the length of the level depending on the start, end, and speed of the level.
 */
 ...
 * @param campaignId the campaign id the level belongs to, set to null if the level don't belong to
 * any campaign
 */
...
/**
 * Sets a new story that is displayed after the level ends, i.e. the player clears the map.
...
/**
 * Sets the base speed of the level. This should only be changed when editing the map, and for the
 * whole level. To change the level's current speed see #Level.setSpeed(float)
