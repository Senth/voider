...
import com.spiddekauga.voider.resource.ResourceUtils;
...
public class ResourceDownloadServlet extends VoiderApiServlet<ResourceDownloadMethod> {
/** Method */
	...
	if (mUser.isLoggedIn()) {
		...
		if (!mParameters.redownload) {
			...
			if (success && !mAddedResources.isEmpty()) {
				...
				ServerMessage<?> serverMessage = new ServerMessage<>(MessageTypes.SYNC_COMMUNITY_DOWNLOAD, mUser.getClientId());
				MessageSender.sendMessage(MessageSender.Receivers.SELF_OTHERS, mUser.getKey(), serverMessage);
			}
		}
		// Redownload
		else {
...
/**
 * Set download date for the specified user so these resources can be synchronized between devices.
 */
 ...
 * @param resourceId id of the resource to get information from and set to the response
...
/**
 * Get information about the specified resource and its dependencies
 * @param resourceKey key of the resource to get information from and set to the response
 * @return true if successfully added information and dependencies
 */
