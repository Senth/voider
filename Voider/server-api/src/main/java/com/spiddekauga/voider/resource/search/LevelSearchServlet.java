...
public class LevelSearchServlet extends ResourceSearchServlet<LevelFetchMethod, LevelDefEntity, LevelInfoEntity> {
private LevelFetchMethod mParameters = null;
private LevelFetchResponse mResponse = null;


public LevelSearchServlet() {
	super(ServerConfig.FetchSizes.LEVELS);
}
...
private void getAndSetLevelResponse() {
	...
	// Just sort, i.e. get the levels
		...
		mLogger.info("Sort");
		mResponse.levels = getLevels(ServerConfig.FetchSizes.LEVELS);
...
private void filterByTags() {
	...
	while (foundLevelsWithTags.size() < ServerConfig.FetchSizes.LEVELS && mResponse.status != FetchStatuses.SUCCESS_FETCHED_ALL) {
		int fetchLimit = (mParameters.tags.size() + 1) * ServerConfig.FetchSizes.LEVELS;
...
@Override
protected LevelInfoEntity datastoreToNetworkEntity(Entity datastoreEntity, LevelInfoEntity networkEntity) {
...
protected FetchStatuses getSuccessStatus(List<?> list) {
	if (list.size() < ServerConfig.FetchSizes.LEVELS) {
...
@Override
protected LevelDefEntity datastoreToDefEntity(Entity datastoreEntity, LevelDefEntity levelDefEntity) {
	super.datastoreToDefEntity(datastoreEntity, levelDefEntity);

	// Level information
	levelDefEntity.levelId = DatastoreUtils.getPropertyUuid(datastoreEntity, CPublished.LEVEL_ID);

	return levelDefEntity;
}
