...
public class EnemySearchServlet extends ActorSearchServlet<EnemyFetchMethod, EnemyDefEntity> {
 ...
 * @return true if we should perform a custom search, if false just get the newest enemies
 */
private boolean hasSearchOptions() {
	if (mParameters.searchString != null && mParameters.searchString.length() >= ServerConfig.SEARCH_TEXT_LENGTH_MIN) {
		return true;
	} else if (!mParameters.movementTypes.isEmpty() && mParameters.movementTypes.size() != MovementTypes.values().length) {
		return true;
	} else if (!mParameters.movementSpeedRanges.isEmpty() && mParameters.movementSpeedRanges.size() != EnemySpeedSearchRanges.values().length) {
		return true;
	} else if (mParameters.hasWeapon != null) {
		return true;
	}
