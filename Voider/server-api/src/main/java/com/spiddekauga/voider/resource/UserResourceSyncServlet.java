...
public class UserResourceSyncServlet extends VoiderApiServlet<UserResourceSyncMethod> {
...
@Override
protected IEntity onRequest(UserResourceSyncMethod userMethod) throws ServletException, IOException {
	if (!mUser.isLoggedIn()) {
	...
	if (mResponse.isSuccessful()) {
		if (!userMethod.resources.isEmpty() || !userMethod.resourceToRemove.isEmpty()
			...
			MessageSender.sendMessage(MessageSender.Receivers.SELF_OTHERS, mUser.getKey(), chatMessage);
		}
	}
