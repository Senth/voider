...
public abstract class ActorSearchServlet<Method extends IMethodEntity, DefType extends DefEntity> extends ResourceSearchServlet<Method, DefType, DefType> {

private String mNextCursor = null;
private FetchStatuses mFetchStatus = null;

protected ActorSearchServlet() {
	super(ServerConfig.FetchSizes.ACTORS);
}
...
protected void getNewestActors(UploadTypes type, String cursor, ArrayList<DefType> actors) {
	...
	// Limit
	FetchOptions fetchOptions = FetchOptions.Builder.withLimit(ServerConfig.FetchSizes.ACTORS);
...
protected FetchStatuses getSuccessStatus(List<?> list) {
	if (list.size() < ServerConfig.FetchSizes.ACTORS) {
