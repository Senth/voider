...
import com.spiddekauga.voider.server.util.DatastoreTables.CPasswordReset;
...
public class PasswordResetServlet extends VoiderApiServlet<PasswordResetMethod> {
 ...
 * @param userKey
 * @param token
 * @return datastore key if token is valid, null if invalid. If invalid the error code will be
 * updated
 */
...
private boolean changePassword(Entity user, String password) {
	if (!RegisterUserServlet.isPasswordLengthValid(password)) {
