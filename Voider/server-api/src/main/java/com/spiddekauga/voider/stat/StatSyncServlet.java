...
import com.spiddekauga.voider.server.util.DatastoreTables.CLevelTag;
...
import com.spiddekauga.voider.server.util.DatastoreTables.CUserLevelStat;
...
public class StatSyncServlet extends VoiderApiServlet<StatSyncMethod> {
// Tables
...
private void syncToServer() {
	...
	if (!mParameters.levelStats.isEmpty()) {
		ServerMessage<?> serverMessage = new ServerMessage<>(MessageTypes.SYNC_STATS, mUser.getClientId());
...
private boolean globalTagsNeedsUpdate(ArrayList<Tags> datastoreTags, Iterable<Field> oldTagFields) {
	...
	return datastoreTags.size() != oldTags.size() || !oldTags.containsAll(datastoreTags);
