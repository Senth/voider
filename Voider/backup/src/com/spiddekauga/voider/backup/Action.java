...
private static final Logger mLogger = Logger.getLogger(Action.class.getSimpleName());
...
protected IEntity uploadToServer(IMethodEntity method, List<FieldNameFileWrapper> files) {
	...
	while (retriesLeft > 0) {
		...
		if (uploadUrlResponse instanceof GetUploadUrlResponse) {
			...
			if (uploadUrl != null) {
				IEntity response = callServerMethod(method, uploadUrl, files);
				if (response != null) {
					return response;
				}
			}
		...
		retriesLeft -= 1;
		mLogger.info("Failed, " + retriesLeft + " retries left");
		try {
			Thread.sleep(10000);
	...
	mLogger.severe("Failed getting an upload url");
	System.exit(1);
	return null;
}
...
private IEntity callServerMethod(IMethodEntity method, String url, List<FieldNameFileWrapper> files) {
	...
	if (entitySend != null) {
		try {
			...
			mLogger.severe("Failed with calling " + method.getClass().getSimpleName() + " on server.");
			return null;
		}
	}
...
protected boolean downloadRequest(IMethodEntity method, String filePath, Date date) {
	...
	if (entity != null) {
		try {
			...
			mLogger.severe("Failed with downloading " + method.getClass().getSimpleName() + " on server.");
			e.printStackTrace();
			System.exit(1);
		}
	}
