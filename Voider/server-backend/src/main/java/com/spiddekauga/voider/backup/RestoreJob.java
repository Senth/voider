 ...
 */
public class RestoreJob extends Job1<Void, Date> {
...
public Value<Void> run(Date restoreTo) throws Exception {
	...
	// Delete Datastore
	DatastoreJobConfig config = new DatastoreJobConfig.Builder()
			...
			.build();
	FutureValue<Void> deleteDatastore = futureCall(new DatastoreDeleteJob(), immediate(config), BackendConfig.getJobSettings());

	// Delete Search
	FutureValue<Void> deleteSearch = futureCall(new DeleteSearchJob(), BackendConfig.getJobSettings());

	// Import Datastore
	FutureValue<Void> restoreDatastore = futureCall(new DatastoreRestoreJob(), immediate(BackupConfig.DATASTORE_BACKUP_CONFIG), BackendConfig.getJobSettings(waitFor(deleteDatastore)));
